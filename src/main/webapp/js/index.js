		...
		$.post("file/searchFile.action",{
			...
			if (data.success)	{
				...
				if (type == "image"){
					$("#"+typeName).empty();
					$.each(data.data,function (){
						var url = encodeURL('currentPath='+this.currentPath+'&fileType='+this.fileType+'&fileName='+this.fileName);
					...
					$.each(data.data,  function (){
						$("#"+  typeName  +"  tbody").append('<tr><td><a  href="#" onclick="return openFile(this)" filetype="'+this.fileType+'" currentPath="'
							+this.currentPath+'"><span class="glyphicon-'+this.fileType+'" style="margin-right: 10px"></span>'
